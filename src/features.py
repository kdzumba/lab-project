...
def term_frequency(text):
    ...
    for dic_term in dictionary.keys():
        ...
        for word in words:
            if word == dic_term:
                count = count + 1
        normalised = (normalised + count * dictionary[dic_term])/(len(text))
...
def dictionary_feature(datapath):
    ...
    for i in range(1, len(data)):
        master_df=pd.concat([master_df, pd.DataFrame(
            term_frequency(data.tweet[i]), index=[i])], ignore_index=True)
...
def tfidf_feature(datapath):
    ...
    count_v=CountVectorizer(ngram_range=(2, 5), min_df=0.001)
