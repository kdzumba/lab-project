...
def term_frequency(text):
    ...
    normalised = 0
    #Count the number of times a word in the dictionary appears in the text
    for dic_term in dictionary.keys():
        count = 0
        for word in words:
            if word == dic_term:
                count = count + 1
        #multiply the number of occurance of a dictionaary word by its weight
        weighted = (normalised + count * dictionary[dic_term])/(count + 1)
        normalised = weighted
...
def dictionary_feature(datapath):
    ...
    df['weights'] = data.tweet.map(term_frequency)
...
if __name__ == '__main__':
    ...
    text = "this is a wonderful morning"
